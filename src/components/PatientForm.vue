...
export default {
  ...
  data() {
    const schema = yup.object({
      ...
      zone: yup.number().required('La zona es requerida'),
  ...
  methods: {
    ...
    async addOrUpdatePatient() {
      ...
      if (this.isEditing) {
        await this.updatePatient(patientToSend);
      } else {
      ...
      <div class="form-group" v-if="isEditing">
        ...
        <ul>
          <li v-for="contact in contactNames(patient.id)" :key="contact.id">
            {{ contact.name }}
            <button @click="editContact(contact.id)" class="btn btn-primary">
              Editar
            </button>
            <button @click="removeContact(contact.id)" class="btn btn-danger">
              Eliminar
            </button>
